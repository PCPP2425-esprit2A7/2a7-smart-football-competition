...
const QRegularExpression MainWindowMatch::scoreRegex("^\\d+-\\d+$");
MainWindowMatch::MainWindowMatch(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindowMatch), model(new QSqlQueryModel(this) )
{
    ...
    btnSuperviser->setGeometry(200, 40, 120, 40);  // x, y, width, height

    // Connexion du signal
    connect(btnSuperviser, &QPushButton::clicked, this, &MainWindowMatch::ouvrirSuperviser);




    connect(ui->pushButton_modifier, &QPushButton::clicked, this, &MainWindowMatch::on_pushButton_modifier_clicked);
    connect(ui->tri, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &MainWindowMatch::on_comboBoxTriDate_currentIndexChanged);
   ...
   calendrierWidget = new CalendrierWidget(this);
    ...
    mapBackground.load(":/img/img/default_map.png"); // Chemin absolu
...
void MainWindowMatch::on_pushButton_exportPDF_clicked() {
...
MainWindowMatch::~MainWindowMatch()
{
    delete model;
    delete ui;
...
void MainWindowMatch::setupTableView()
{
    ui->tableView->setModel(model);
    ui->tableView->setSelectionBehavior(QAbstractItemView::SelectRows);
    ui->tableView->setSelectionMode(QAbstractItemView::SingleSelection);
...
void MainWindowMatch::loadMatchData() {
...
void MainWindowMatch::on_tableView_clicked(const QModelIndex &index)
{
    ...
    ui->date_time->setDateTime(dt.isValid() ? dt : QDateTime::currentDateTime());

    ui->place->setText(model->data(model->index(row, 2)).toString());
    ui->score->setText(model->data(model->index(row, 3)).toString());
    ui->status->setCurrentText(model->data(model->index(row, 4)).toString());
    ui->arbitre->setText(model->data(model->index(row, 5)).toString());
...
// void MainWindowMatch::on_pushButton_ajouter_clicked() {
//     // Get values from UI
//     QDateTime datetime = ui->date_time->dateTime();
//     QString place = ui->place->text().trimmed();
...
//     bool arbitreOk;
//     int arbitre = ui->arbitre->text().toInt(&arbitreOk);
...
void MainWindowMatch::on_pushButton_ajouter_clicked()
{
    ...
    if (!ui->date_time || !ui->place || !ui->score || !ui->status || !ui->team1 || !ui->team2 || !ui->arbitre) {
    ...
    // Conversion de l'ID de l'arbitre
    bool arbitreOk;
    int arbitre = ui->arbitre->text().toInt(&arbitreOk);

    // Validation minimale
    if (place.isEmpty() || score.isEmpty() || status.isEmpty() || team1.isEmpty() || team2.isEmpty() || !arbitreOk) {
        QMessageBox::warning(this, "Erreur", "Veuillez remplir tous les champs correctement.");
        return;
    }

    // Création de l'objet Match (avec ID 0 par défaut)
    Match m(
        0,
    ...
    if (m.ajouterMatch()) {
        ...
        clear();         // Réinitialiser les champs
    } else {
...
void MainWindowMatch::on_pushButton_modifier_clicked()
{
    ...
    if (ui->place->text().trimmed().isEmpty())
        errors << "Le lieu doit être renseigné";

    if (ui->score->text().trimmed().isEmpty())
        errors << "Le score doit être renseigné";
    else if (!QRegularExpression("^\\d+-\\d+$").match(ui->score->text()).hasMatch())
        errors << "Format de score invalide (doit être X-Y)";

    if (ui->status->currentText().isEmpty())
    ...
    qDebug() << "ID:" << id;
    qDebug() << "Date:" << ui->date_time->dateTime().toString("yyyy-MM-dd HH:mm:ss");
    qDebug() << "Lieu:" << ui->place->text().trimmed();
    qDebug() << "Score:" << ui->score->text().trimmed();
    qDebug() << "Statut:" << ui->status->currentText();
    ...
    bool result = m.modifierMatch(id,
                                  ui->date_time->dateTime().toString("yyyy-MM-dd HH:mm:ss"),
                                  ui->place->text().trimmed(),
                                  ui->score->text().trimmed(),
                                  ui->status->currentText(),
                                  arbitre,
                                  team1,
                                  team2);
...
void MainWindowMatch::on_pushButton_supprimer_douaa_clicked()
{
...
void MainWindowMatch::on_pushButton_rechercher_clicked()
{
...
void MainWindowMatch::on_pushButton_calculer_clicked()
{
...
bool MainWindowMatch::teamExists(int teamId) {
    QSqlQuery query;
...
void MainWindowMatch::on_comboBoxTriDate_currentIndexChanged(int index)
{
    switch(index) {
    case 1: // Plus ancien
...
void MainWindowMatch::chargerDonneesTriees(const QString& order)
{
    QSqlQueryModel *model = new QSqlQueryModel(this);
...
/*void MainWindowMatch::on_pushButton_superviser_clicked()
{
...
void MainWindowMatch::ouvrirSuperviser() {
...
void MainWindowMatch::on_dark_clicked()
{
    ...
    ui->pushButton_quitter->setStyleSheet("#pushButton_quitter{border-radius: 10px;background: transparent;}");
...
void MainWindowMatch::on_light_clicked()
{
...
void MainWindowMatch::on_supp_btn_clicked()
{
...
void MainWindowMatch::on_joueur_btn_clicked()
{
    this->close(); // Cache la fenêtre actuelle (facultatif)
    MainWindowJoueur *w1 = new MainWindowJoueur(); // Crée une nouvelle fenêtre
...
void MainWindowMatch::on_arbitre_btn_clicked()
{
    this->close(); // Cache la fenêtre actuelle (facultatif)
...
void MainWindowMatch::on_billet_btn_clicked()
{
    this->close(); // Cache la fenêtre actuelle (facultatif)
    MainWindowBillet *w4 = new MainWindowBillet(); // Crée une nouvelle fenêtre
    w4->setAttribute(Qt::WA_DeleteOnClose); // Pour éviter les fuites mémoire
    w4->show(); // Affiche la nouvelle fenêtre
}


void MainWindowMatch::on_home_clicked()
{
    this->close(); // Cache la fenêtre actuelle (facultatif)
...
void MainWindowMatch::on_equipe_btn_clicked()
{
    this->close(); // Cache la fenêtre actuelle (facultatif)
