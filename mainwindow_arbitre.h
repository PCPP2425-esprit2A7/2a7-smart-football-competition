...
namespace Ui {
class MainWindowArbitre;
}
class Statistique;
class MainWindowArbitre : public QMainWindow
{
    Q_OBJECT

public:
    explicit MainWindowArbitre(QWidget *parent = nullptr); // Constructeur
    ~MainWindowArbitre(); // Destructeur
...
private slots:
    ...
    void on_bouton_billet_arbitre_clicked();

    void on_bouton_supporteur_arbitre_clicked();

    void on_bouton_joueur_arbitre_clicked();

    void on_bouton_match_arbitre_clicked();

    void on_home_clicked();

    void on_bouton_equipe_arbitre_clicked();

private:
    Ui::MainWindowArbitre *ui; // Objet UI généré par Qt Designer

    QProcess *voiceProcess;
    //Statistique *stat;
    DialogArbitre *dialog_window;
